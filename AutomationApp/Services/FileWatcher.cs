...
namespace AutomationApp.Services
{
    public class FileWatcherService(FileService fileService)
    {
        ...
        public async Task StartWatching(string folderPath, string fileType, string destination)
        {
            ...
            _watcher.Created += async (sender, e) =>
            {
                ...
                await _fileService.MoveFileAsync(e.FullPath, Path.Combine(destination, Path.GetFileName(e.FullPath)));
            };
